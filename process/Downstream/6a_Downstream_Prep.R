...
# spe@metadata$markers = yaml::read_yaml("data/downstream/IMC_cell_type_markers.yml")
# 
# spe@metadata$color_vectors$all_anno = NULL
# spe@metadata$color_vectors$high_level_anno = NULL
# 
# # Surgery
# spe@metadata$color_vectors$surgery = c("#4F94CD","#CD4F39")
# names(spe@metadata$color_vectors$surgery) = unique(spe$surgery)
# 
# # Patient
# spe@metadata$color_vectors$patient = viridis(n = length(unique(spe$patient)),
#                                              option ="turbo")
# names(spe@metadata$color_vectors$patient) = unique(spe$patient)
# 
# # ROI
# spe@metadata$color_vectors$ROI = viridis(n = length(unique(spe$ROI)),
#                                          option ="turbo")
# names(spe@metadata$color_vectors$ROI) = unique(spe$ROI)
# 
# # Region type
# spe@metadata$color_vectors$region_type = viridis(n = length(unique(spe$region_type)),
#                                                  option ="turbo")
# names(spe@metadata$color_vectors$region_type) = unique(spe$region_type)
# spe@metadata$color_vectors$region_type["unknown"] = "grey70"


# Cell Type Annotations
# spe@metadata$cell_types = list(
#   immune = list(lymphoid = c("T cells", "NK cells"),
#                 myeloid = c("Microglia", "Macrophage")),
#   tumour = c("AC", "MES", "NPC", "OPC", "Hybrid"),
#   stroma = c("Neuron", "Astrocyte", "Oligodendrocyte", "Endothelial")
# )
# 
# spe$cell_anno = ifelse(spe@colData$cell_anno == 'AC_MES', "Hybrid",
#                        spe@colData$cell_anno)
# 
# spe$cell_anno = factor(spe$cell_anno,
#                        levels = c(unlist(spe@metadata$cell_types$immune),
#                                   spe@metadata$cell_types$tumour,
#                                   spe@metadata$cell_types$stroma))
# 
# spe@metadata$color_vectors$cell_anno = viridis(n = length(levels(spe$cell_anno)),direction = -1, option = "turbo")
# names(spe@metadata$color_vectors$cell_anno) = levels(spe$cell_anno)


# spe@metadata$cell_types$immune$lymphoid = c("T cell", "NK cell")

# Main Annotations
# spe$main_anno = as.character(spe$manual_gating)
# 
# spe$main_anno[spe$main_anno %in% unlist(spe@metadata$cell_types$immune)] = "Immune"
# spe$main_anno[spe$main_anno %in% spe@metadata$cell_types$tumour] = "Tumour"
# spe$main_anno[spe$main_anno %in% spe@metadata$cell_types$stroma] = "Stroma"
# spe$main_anno[spe$main_anno %in% "undefined"] = NA
# 
# spe$main_anno = factor(spe$main_anno, levels = c("Immune", "Tumour","Stroma"))

# spe@metadata$color_vectors$main_anno = viridis(n = length(levels(spe$main_anno)),
#                                                option ="turbo", direction = -1)
# names(spe@metadata$color_vectors$main_anno) = levels(spe$main_anno)

# COMPARE CELL COVERAGES BETWEEN OLD AND NEW LABELS  ---------------------------

old_spe <- readRDS("../Data/Downstream_Analysis/spe_comp.rds")

new_labels = table(spe@colData$manual_gating) %>%
  as.data.frame() %>%
  mutate(new_anno = round(Freq/ncol(spe) * 100, 3))


undefined_anno = data.frame(Var1 = "undefined",
           Freq = length(which(is.na(spe$manual_gating))),
           new_anno = 100 - sum(new_labels$new_anno))

new_labels = rbind(new_labels, undefined_anno)
  
 
rm(undefined_anno)

old_spe$manual_anno = ifelse(is.na(old_spe$manual_anno), "undefined", old_spe$manual_anno)

old_labels = table(old_spe@colData$manual_anno) %>%
  as.data.frame() %>%
  mutate(old_anno = round(Freq/ncol(old_spe) * 100, 3))

old_labels$Var1 = str_replace_all(old_labels$Var1, "Cells", "cell") %>%
  str_replace_all(., "Macrophages", "Macrophage") %>%
  str_replace_all(., "Monocytes", "Monocyte")

anno_comp = left_join(new_labels, old_labels, by = "Var1") %>%
  select(-c("Freq.x", "Freq.y")) %>%
  tidyr::pivot_longer(2:3,names_to = "cell_type", values_to = "percent")

io$plots$anno_comp = anno_comp %>%
  ggplot(aes(fill=cell_type, y=percent, x=Var1)) +
  geom_bar(position="dodge", stat="identity") +
  xlab("") +
  ylab("percent of total cells") +
  theme_classic()

ggsave(plot = io$plots$anno_comp,
       path = file.path(io$output$cell_pheno),
       width = 15,
       filename = nf("annotation_comp.svg"),
       device = "svg")


rm(new_labels, old_labels, old, old_spe, out, anno_comp)

rm(anno_comp, new_labels, old_labels, out)

# LABELLED DIMPLOT  ------------------------------------------------------------

# Alter the color vector names and remove the hybrid cell anno
names(spe@metadata$color_vectors$cell_anno) = gsub("(?i)s$","", names(spe@metadata$color_vectors$cell_anno))
spe@metadata$color_vectors$cell_anno = spe@metadata$color_vectors$cell_anno[!names(spe@metadata$color_vectors$cell_anno) %in% "Hybrid"]
spe@metadata$color_vectors$cell_anno


svglite::svglite(
  file = nf("labelled_UMAP_Dimplot.svg", io$output$cell_pheno),
  width = 15,
  height = 12
)

dittoSeq::dittoDimPlot(spe, 
             reduction.use = "UMAP_harmony_minmax",
             var = "manual_gating",
             color.panel = spe@metadata$color_vectors$cell_anno,
             size = 0.5,
             opacity = 0.9,
             main = "") +
...
ggplot(plot_data, 
  ...
  scale_fill_manual(values = spe@metadata$color_vectors$region_type, 
...
region_counts %>%
  ...
  scale_fill_manual(values = setNames(c("#4F94CD","#CD4F39"),
                                      c("Primary", "Recurrent"))) +
...
outfile = sort(list.files(io$inputs$comp_data, pattern = "spe_comp_\\d{4}",
                          full.names = T),
     decreasing = T)[1]
...
prevelances = prevelances %>%
  ...
  tidyr::separate(col = sample_id, into = c("patient", "surgery", "region"),
                  sep = "_", remove = FALSE) %>%
...
io$plot_params <- data.frame(x_val = rep(c("surgery","region_type","responder_type"), 
                                      c(10,10,5)),
                          fill = "cell_type",
...
io$plot_params <- rbind( 
  data.frame(x_val = rep(c("surgery","region_type","responder_type"), 
                         c(4,4,2)),
...
spatial_anno_cols = c(spe@metadata$color_vectors$main_anno,
                       spe@metadata$color_vectors$cell_anno)
...
pdf(onefile = T, width = 15, height = 15, 
...
aggregate_prevalences(prevalence_df = prevelances,
                      main_group = "surgery",
...
split_prevalences <- function(data, group_by = NULL, 
  ...
  if(return_plot){
   ...
   plots <- purrr::imap(split_data, ~{
     ...
     n_fill_colors = ncol(.x) - 2
      ...
      outplot <- .x %>% 
        tidyr::pivot_longer(cols = -c("region","cell_type"), 
  ...
  } else return(split_data)
  
  
}


split_data <- setNames(c("surgery","region_type","responder_type"),  
                       c("surgery","region_type","responder_type")) |> 
              as.list() |>
              purrr::map(~split_prevalences(data = prevelances, 
...
compare_two_groups <- function(data){
  ...
  test <- wilcox.test(x = data[[cols[1]]], 
                      y = data[[cols[2]]], 
                      paired = T, 
                      conf.int = T, # with CIs
  ...
  effect_data <- data %>% tidyr::pivot_longer(all_of(cols)) %>% arrange(name, region)
  
  effect <- rcompanion::wilcoxonPairedR(x  = effect_data$value, 
                                        g  = effect_data$region, ci = T)
  
  out <- data.frame(celltype = unique(data$cell_type), 
...
compare_two_plus_groups <- function(data){
  ...
  comp_stats <- function(data, comps){
    ...
    # Paired Wilcoxon Signed Rank Tests
    test <- wilcox.test(x = data[[comps[1]]], 
                        y = data[[comps[2]]], 
                        paired = T, 
                        conf.int = T, # with CIs
                        exact = F # gets rid of obnoxious warnings
    ...
    effect_data <- tidyr::pivot_longer( data = data[c("region","cell_type", comps)], 
                                        cols = all_of(comps)) %>% 
      arrange(name, region)
...
split_data$surgery$stats <- purrr::map(split_data$surgery$data, 
                                       ~compare_two_groups(.x)) |> bind_rows()

split_data$responder_type$stats <- purrr::map(split_data$responder_type$data, 
                                              ~compare_two_groups(.x)) |> bind_rows()


split_data$region_type$stats <- purrr::map(split_data$region_type$data, 
                                              ~compare_two_plus_groups(.x)) |> bind_rows()


split_data$prim_region_type$stats <- purrr::map(split_data$prim_region_type$data, 
                                           ~compare_two_plus_groups(.x)) |> bind_rows()


split_data$rec_region_type$stats <- purrr::map(split_data$rec_region_type$data, 
                                           ~compare_two_plus_groups(.x)) |> bind_rows()


split_data$prim_responder_type$stats <- purrr::map(split_data$prim_responder_type$data, 
                                                ~compare_two_groups(.x)) |> bind_rows()


split_data$rec_responder_type$stats <- purrr::map(split_data$rec_responder_type$data, 
                                                   ~compare_two_groups(.x)) |> bind_rows()
...
io$prevalances$data <- map(split_data, ~{pluck(.x, "data")})

io$prevalances$plots <- map(split_data, ~{pluck(.x, "plots")})
...
purrr::iwalk(io$prevalances$data, ~{
  ...
  openxlsx::write.xlsx(.x, 
                       file = file.path("Outputs/Spatial_Analysis/Prevalences",
                                        filename))
...
openxlsx::write.xlsx(io$prevalances$stats, 
                     file = file.path(io$output$cell_prevalences, 
...
# clear memory
rm(split_data, compare_two_groups, compare_two_plus_groups, split_prevalences,
...
for (i in seq(5, 25, 5)) {
  ...
  spe <- buildSpatialGraph(spe, img_id = "ImageName", type = "knn", k = i, 
...
interactions <- set_names(c("surgery","responder_type","region"),
                          c("surgery","responder_type","region")) %>% 
  as.list() %>% 
  purrr::map(~ testInteractions(object = spe, 
                                group_by = .x,
                                label = "all_v2",
                                method = "histocat",
                                p_threshold = 0.05,
...
openxlsx::write.xlsx(interactions_2, 
